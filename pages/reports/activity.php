...
function showJob(str) {
    if (str == "") {
        ...
        xmlhttp.open("GET","../scripts/active.php?q="+str,true);
        xmlhttp.send();
    }
}
</script>
   ...
   <title>ATS: Recruiter Post History</title>
</head>
<body>
   <div id="wrapper">
<div class="page">
...
<form id="report" name="report" method="post" action="">
        <label id="reportJob" name="reportJob">Recruiters Post history to lookup:</label>
          <input type="text" id="recruiter" name="recruiter" onkeydown="if (event.keyCode == 13) document.getElementById('btnSubmit').click()"/>
          <input type="button" value="submit" id="btnSubmit" onclick="showJob(document.getElementById('recruiter').value)" />
        </form>
        <br />
