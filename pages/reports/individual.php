...
function showJob(str) {
    if (str == "") {
        ...
        xmlhttp.open("GET","../scripts/indiv.php?q="+str,true);
        xmlhttp.send();
    }
}
</script>
   ...
   <title>ATS: Posted Job Report</title>
</head>
<body>
   <div id="wrapper">
<div class="page">
...
<form id="report" name="report" method="post" action="">
        <label id="reportJob" name="reportJob">Job requisition to lookup:</label>
          <input type="text" id="jobreq" name="jobreq" onkeydown="if (event.keyCode == 13) document.getElementById('btnSubmit').click()"/>
          <input type="button" value="submit" id="btnSubmit" onclick="showJob(document.getElementById('jobreq').value)" />
        </form>
        <br />
